        ...
        case x_char:
            {
				...
				*dest = *sqlite3_column_text(statement_.stmt_, pos);
                //*dest = *buf;
            }
            break;
        case x_stdstring:
            {
				...
				dest->assign(reinterpret_cast<const char*>(sqlite3_column_text(statement_.stmt_, pos)));
                //dest->assign(buf);
            }
            break;
        case x_short:
        ...
        case x_long_long:
            {
				...
				*dest = sqlite3_column_int64(statement_.stmt_, pos);
        ...
        case x_double:
            {
				...
				*dest = sqlite3_column_double(statement_.stmt_, pos);
        ...
        case x_stdtm:
            {
				...
				parse_std_tm(reinterpret_cast<const char*>(sqlite3_column_text(statement_.stmt_, pos)), *dest);
            }
            break;
        case x_rowid:
            {
...
//                 rbe->value_ = static_cast<unsigned long>(val);
				rbe->value_ = sqlite3_column_int(statement_.stmt_, pos);
			}
            break;
        case x_blob:
            {
