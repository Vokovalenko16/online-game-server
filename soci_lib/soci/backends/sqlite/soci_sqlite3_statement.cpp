        ...
        for (i = 0; i < totalRows && databaseReady_; ++i)
        {
            ...
            else if (SQLITE_ROW == res)
            {
                ...
                for (int c = 0; c < numCols; ++c)
                {
                    ...
                    dataCache_[i][c].type_ = isNull ? sqlite3_c_null : sqlite3_c_text;
                }
            }
            else
            {
...
sqlite3_statement_backend::load_one()
{
    ...
    {
//        clean_up(); [Super Faker : 2013/07/20] : With this any error during statement execution makes the statement unusable.
...
sqlite3_statement_backend::bind_and_execute(int number)
{
    ...
    for (int row = 0; row < rows; ++row)
    {
        ...
        for (int pos = 1; pos <= totalPositions; ++pos)
        {
			...
			case sqlite3_c_int:
				bindRes = sqlite3_bind_int(stmt_, pos, curCol.numeric.i);
				break;

			case sqlite3_c_int64:
				bindRes = sqlite3_bind_int64(stmt_, pos, curCol.numeric.ll);
				break;

			case sqlite3_c_double:
				bindRes = sqlite3_bind_double(stmt_, pos, curCol.numeric.d);
				break;

			default:
				throw soci_error("Internal library error");
			}
