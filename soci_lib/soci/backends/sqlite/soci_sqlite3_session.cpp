...
void sqlite3_session_backend::begin(int level)
{
	std::ostringstream ss;
	ss << "SAVEPOINT s_" << level;
	execude_hardcoded(conn_, ss.str().c_str(), "Cannot begin nested transaction.");
}

void sqlite3_session_backend::commit(int level)
{
	std::ostringstream ss;
	ss << "RELEASE s_" << level;
	execude_hardcoded(conn_, ss.str().c_str(), "Cannot commit nested transaction.");
}

void sqlite3_session_backend::rollback(int level)
{
	std::ostringstream ss;
	ss << "ROLLBACK TO s_" << level << "; RELEASE s_" << level;
	execude_hardcoded(conn_, ss.str().c_str(), "Cannot rollback nested transaction.");
}
