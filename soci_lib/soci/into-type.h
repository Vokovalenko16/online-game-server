...
namespace detail_sf
{
	...
	true_type is_basic_type(basic_type_tag);
	false_type is_basic_type(user_type_tag);
}
...
public:
	...
	{
		static_assert(
			sizeof (detail_sf::is_basic_type(typename exchange_traits<T>::type_family())) == sizeof (detail_sf::true_type),
