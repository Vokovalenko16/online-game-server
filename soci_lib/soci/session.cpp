...
void session::begin(int level)
{
	backEnd_->begin(level);
}

void session::commit(int level)
{
	backEnd_->commit(level);
}

void session::rollback(int level)
{
	backEnd_->rollback(level);
}
